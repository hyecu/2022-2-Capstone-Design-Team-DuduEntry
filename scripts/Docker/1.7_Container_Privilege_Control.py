...
title="1.7. Docker Configuration - Container Privilege Control"
m.BAR()
...
print("Step1. suid/sgid 제한 1) docker daemon 설정") # '--no-new-privileges' 값이 적절하게 설정되어 있는지 확인
daemon_suid_sgid = os.popen("ps -ef | grep dockerd | grep -v grep").read()
if (daemon_suid_sgid.find('no-new-privileges')) == -1:
    res=0
ret = m.res_chk(res, vul, res_init, title, "\
Step1. suid/sgid 제한 1) docker daemon 설정\n\
...
res=1
print("Step2. suid/sgid 제한 2) docker runtime 설정") # '--security-opt' 값이 적절하게 설정되어 있는지 확인
runtime_suid_sgid = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:SecurityOpt={{ .HostConfig.SecurityOpt }}'").read()
if (runtime_suid_sgid.find('no-new-privileges')) == -1:
    res=0 # suid/sgid 제한 - docker runtime 설정 : 취약
ret = m.res_chk(res, vul, res_init, title, "\
Step2. suid/sgid 제한 2) docker runtime 설정\n\
...
experimental = os.popen("docker version --format '{{ .Server.Experimental }}'").read()
...
res=1
print("Step4. cgroup 변경 금지 1) docker daemon 설정") # '--cgroup-parent' 및 적절한 cgroup 설정 여부 확인
daemon_cgroup = os.popen("ps -ef | grep dockerd | grep -v grep").read()
if (re.search("--cgroup-parent=\W\w+",daemon_cgroup)) is not None:
    res=0
ret = m.res_chk(res, vul, res_init, title, "\
Step4. cgroup 변경 금지 1) docker daemon 설정\n\
...
print("Step5. cgroup 변경 금지 2) docker runtime 설정")
runtime_cgroups = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .HostConfig.CgroupParent }}'").read().splitlines()
    ...
    if re.search("\w+",runtime_cgroup) is not None:
        for cgroup in cgroup_list:
            if (runtime_cgroup.find(cgroup)) == -1:
                ...
                break
ret = m.res_chk(res, vul, res_init, title, "\
Step5. cgroup 변경 금지 2) docker runtime 설정\n\
...
privileged = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .HostConfig.Privileged }}'").read()
...
root_filesystem = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:ReadonlyRootfs={{ .HostConfig.ReadonlyRootfs }}' | awk -F '=' '{print $2}'").read()
