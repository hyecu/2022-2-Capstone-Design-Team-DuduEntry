...
title = "1.4. Docker Configuration - Management Namespace"
m.BAR()
...
for i in range(0,5):
    ...
    checks = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:"+modes[i]+"={{ .HostConfig."+modes[i]+" }}'").read().splitlines()
    res=1
    for check in checks:
        if 'host' in check:
            res=0
...
container_ID = os.popen("docker ps | awk '{print $1}'").read()
lines = len(container_ID.splitlines())
for i in range(1,lines):
    container_user = os.popen("ps -p $(docker inspect --format='{{ .State.Pid }}' "+container_ID.splitlines()[i]+") -o pid,user").read()
