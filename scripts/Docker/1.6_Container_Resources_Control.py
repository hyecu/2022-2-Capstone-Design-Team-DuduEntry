...
title="1.6. Docker Configuration - Container Resources Control"
m.BAR()
...
check_storage = os.popen("ps -ef | grep dockerd | grep -v grep").read()
...
shared_volumes = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{range $mnt := .Mounts}} {{json $mnt.Propagation}} {{end}}'").read().splitlines()
...
memories = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:Memory={{ .HostConfig.Memory }}' | awk -F '=' '{print $2}'").read().splitlines()
...
cpu_shares = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:CpuShares={{ .HostConfig.CpuShares }}' | awk -F '=' '{print $2}'").read().splitlines()
...
res=1
print("Step5. 프로세스 제한 설정 1) docker daemon 설정") # '--default-ulimit' 파라미터값이 적절하게 설정되어 있는지 확인
daemon_process_limit = os.popen("ps -ef | grep dockerd | grep -v grep").read()
if (daemon_process_limit.find('default-ulimit')) == -1:
    res=0
ret = m.res_chk(res, vul, res_init, title, "\
Step5. 프로세스 제한 설정 1) docker daemon 설정\n\
--default-ulimit 파라미터값이 적절하게 설정되어 있지 않습니다.\n\
Docker 서비스 실행 시 아래의 명령어를 통해 프로세스 제한을 설정해 주세요.\n\
    dockerd --default-ulimit nproc=1024:2048 --default-ulimit nofile=100:200")
res_init = ret[0]; vul = ret[1]

res=1
print("Step6. 프로세스 제한 설정 2) docker runtime 설정")
runtime_process_limits = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:Ulimits={{ .HostConfig.Ulimits }}' | awk -F '=' '{print $2}'").read().splitlines()
for runtime_process_limit in runtime_process_limits:
    if (runtime_process_limit.find("no value")) != -1:
        res=0
        break
ret = m.res_chk(res, vul, res_init, title, "\
Step6. 프로세스 제한 설정 2) docker runtime 설정\n\
Ulimits가 적절하게 설정되어 있지 않습니다.\n\
아래의 명령어를 통해 Docker daemon에서 설정한 ulimit을 무시하도록 설정해 주세요.\n\
    docker run --ulimit nofile=1024:1024 --interactive --tty centos /bin/bash")
res_init = ret[0]; vul = ret[1]

res=1
print("Step7. 프로세스 제한 설정 3) 프로세스 생성 제한")
process_limits = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:PidsLimit={{ .HostConfig.PidsLimit }}' | awk -F '=' '{print $2}'").read().splitlines()
...
restart_policies = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:RestartPolicyName={{ .HostConfig.RestartPolicy.Name }}' | awk -F '=' '{print $2}'").read().splitlines()
retry_counts = os.popen("docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}:MaximumRetryCount={{ .HostConfig.RestartPolicy.MaximumRetryCount }}' | awk -F '=' '{print $2}'").read().splitlines()
