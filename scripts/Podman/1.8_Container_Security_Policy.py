...
title="1.8 Podman Configuration - Container Security Policy"
m.BAR()
m.CODE(title) # 컨테이너 보안 정책
m.BAR()

print("Step1. seccomp 프로필 사용 여부")
use_seccomp_profiles = os.popen("podman ps --quiet --all | xargs podman inspect --format '{{ .HostConfig.SecurityOpt }}'").read().splitlines()
...
apply_seccomp_profile = os.popen("podman info --format '{{ .SecurityOptions }}'").read()
...
아래의 명령어를 통해 seccomp 프로필을 적용해 주세요.\n\
    podmand --seccomp-profile <seccomp 프로파일 파일 경로>")
res_init = ret[0]; vul = ret[1]

res=1
print("Step3. AppArmor 프로필 활성화")
AppArmor_profiles = os.popen("podman ps --quiet --all | xargs podman inspect --format '{{ .AppArmorProfile }}'").read().splitlines()
...
SELinux = os.popen("podman ps --quiet --all | xargs podman inspect --format '{{ .HostConfig.SecurityOpt }}'").read()
if (SELinux.find("no value")) == -1: # 'no value' 가 아니면 양호
    res=0
ret = m.res_chk(res, vul, res_init, title, "\
Step4. SELinux 사용\n\
SELinux를 사용하고 있지 않습니다.\n\
SELinux가 활성화 된 상태에서 아래와 같이 실행시킨 후, podman daemon --selinux-enabled\n\
컨테이너 실행 시 아래와 같은 옵션을 사용해 주세요.\n\
    podman run --interactive --tty --security-opt label=level:TopSecret centos /bin/bash")
res_init = ret[0]; vul = ret[1]

res=1
print("Step5. 컨테이너 내 리눅스 커널 Capabilities 제한")
CapAdd = os.popen("podman ps --quiet --all | xargs podman inspect --format '{{ .HostConfig.CapAdd }}'").read()
CapDrop = os.popen("podman ps --quiet --all | xargs podman inspect --format '{{ .HostConfig.CapDrop }}'").read()
...
컨테이너 실행 시 사용 환경에 맞게 아래와 같이 기능 추가 및 제거:\n\
    # 필요한 Capabilities 추가\n\
    podman run --cap-add={'Capability 1','Capability 2'}\n\
    # 필요하지 않은 Capabilities 삭제\n\
    podman run --cap-drop={'Capability 1','Capability 2'}\n\
    # 모든 Capabilities 삭제 후 필요한 Capabilities 추가\n\
    podman run --cap-drop=all --cap-add={'Capability 1','Capability 2'}")
    m.END()
