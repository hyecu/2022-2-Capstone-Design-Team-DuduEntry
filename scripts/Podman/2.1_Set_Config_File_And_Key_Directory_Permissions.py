...
podman_files = ['service', 'socket']
files = ['', '', '/var/run/podman.sock', '/etc/podman/daemon.json', '/etc/podman', '/etc/default/podman']
check_files = ['/usr/lib/systemd/system/podman.service', '/usr/lib/systemd/system/podman.socket']
perm = ['644', '644', '660', '644', '755', '644']


for i in range(0,2):
    files[i] = os.popen("systemctl show -p FragmentPath podman."+podman_files[i]+" | sed 's/^.*=//'").read().strip()
    if (files[i][:4]) != "/usr":
        files[i] = "/usr" + files[i]
...
파일의 경로가 맞지않거나 파일 소유 및 권한이 적절하게 설정되지 않았습니다.\n\
    podman.service의 경로는 "+check_files[0]+"\n\
    podman.socket의 경로는 "+check_files[1]+"로 설정해 주세요.\n\
