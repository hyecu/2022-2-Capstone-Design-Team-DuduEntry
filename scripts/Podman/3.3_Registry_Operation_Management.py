...
content_trust = os.popen("echo $PODMAN_CONTENT_TRUST").read()
...
registry_encryption = os.popen("ps -ef | grep podmand").read()
...
version_check = os.popen("podman version --format '{{.Server.Version}}'").read()
...
for i in range(len(ver_num)):
    if int(ver_num[i]) > int(v17_12[i]):
        ...
        check_old_ver = os.popen("ps -ef | grep podman").read()
...
아래의 명령어를 사용하여 비 활성화해 주세요.\n\
    podmand --disable-legacy-registry")
res_init = ret[0]; vul = ret[1]
